...
if (!class_exists("Shiprocket_Woocommerce_Shipping_Method")) {
    ...
    class Shiprocket_Woocommerce_Shipping_Method extends WC_Shipping_Method {
        ...
        private function init() {
            ...
            self::$fallback_rate = !empty($this->settings['fallback_rate']) ? $this->settings['fallback_rate'] : false;
            $this->shipping_title = !empty($this->settings['shipping_title']) ? $this->settings['shipping_title'] : false;
            self::$flat_rate = !empty($this->settings['flat_rate']) ? $this->settings['flat_rate'] : false;
        ...
        public function calculate_shipping($package = array()) {
            if ($this->realtime_enabled == "yes") {
                ...
                if (empty($this->found_rates) && self::$fallback_rate && $this->shipping_title) {
        ...
        public function prepare_rate($shipping_method_detail) {
            ...
            if ($shipping_method_detail->etd != '' && ($shipping_method_detail->carrierId == "fallback_rate" || $shipping_method_detail->carrierId == "flat_rate" )) {
